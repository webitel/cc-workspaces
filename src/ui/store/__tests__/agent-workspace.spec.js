...
describe('workspace store: actions', () => {
	...
	it('SET_WORKSPACE_STATE commits ADD_WORKSPACE_STATE with passed state', () => {
		const task = {};
		workspaceModule.actions.SET_WORKSPACE_STATE(context, {
			type: WorkspaceStates.CALL,
			task,
		});
		expect(context.commit).toHaveBeenCalledWith('ADD_WORKSPACE_STATE', {
			type: WorkspaceStates.CALL,
			task,
		});
	});
