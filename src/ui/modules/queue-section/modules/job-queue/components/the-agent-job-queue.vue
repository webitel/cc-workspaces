...
const distributedTasks = computed(() =>
	taskList.value.filter((task) => task.state === JobState.Offering),
);
const activeTasks = computed(() =>
	taskList.value.filter((task) => task.state !== JobState.Offering),
);
const allActiveTasks = computed(
	() => activeTasks.value.length + distributedTasks.value.length,
);
