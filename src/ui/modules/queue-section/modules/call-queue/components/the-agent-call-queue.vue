...
const ringingCallsCount = computed(
	() => callList.value.filter((call) => isIncomingRinging(call)).length,
);
const activeCallsCount = computed(
	() => callList.value.length - ringingCallsCount.value,
);
const allActiveCalls = computed(
	() => activeCallsCount.value + ringingCallsCount.value,
);
...
const expansionStates = ref({
	...
	missed: restoreExpansionState({
		expansion: 'missed',
	}),
	offline: restoreExpansionState({
		expansion: 'offline',
	}),
...
const expansions = computed(() => [
	{
		...
		counters: [
			...
			{
				color: 'main',
				count: activeCallsCount.value,
			},
			{
				color: 'success',
				count: ringingCallsCount.value,
			},
		].filter(({ count }) => count),
	},
	{
		...
		counters: [
			{
				color: 'secondary',
				count: missedNext.value
					? `${missedList.value.length}+`
					: missedList.value.length,
			},
		].filter(({ count }) => count),
	},
	{
		...
		counters: [
			{
				color: 'secondary',
				count: membersList.value.length,
			},
		].filter(({ count }) => count),
	},
	{
