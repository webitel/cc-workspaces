...
describe('Footer buttons', () => {
	...
	it('Mutes call', () => {
		...
		expect(mute.classes()).not.toContain('hidden');
		mute.vm.$emit('click');
		expect(callOnWorkspace.mute).toHaveBeenCalledWith(!callOnWorkspace.muted);
	});
	...
	it('Holds call', () => {
		...
		expect(hold.classes()).not.toContain('hidden');
		hold.vm.$emit('click');
		expect(callOnWorkspace.toggleHold).toHaveBeenCalled();
	});
});
