...
const getters = {
	AGENT_ID: (state, getters, rootState) =>
		rootState.features.status.agent.agentId, // used for initial agent data loading
	IS_DESC_TRACK_AUTH_POPUPS_ALLOW: (state, getters, rootState, rootGetters) => {
		return (
			state.agent?.screenControl &&
...
const actions = {
	...
	LOAD_PAUSE_CAUSES: async (context) => {
		const { items } = await AgentPauseCausesAPI.getList({
			agentId: context.getters.AGENT_ID,
		});
