...
const getList = async ({ path, filters, ...params }) => {
  const listResponseHandler = (items) => {
    return items.map((item) => {
      // @liza because of name:{common_name: 'Contact'} in contact object https://webitel.atlassian.net/browse/WTEL-6797
  ...
  try {
    ...
    return {
      ...
      items: applyTransform(data || items, [listResponseHandler]),
      next,
    };
  } catch (err) {
