...
export default function getNestedValue(value, pathArray: string[]) {
	if (value === 0) return 0; // 0 is valid value
	...
	if (Array.isArray(value)) {
		// if value is array, we need to iterate every item in array and get nested value from EVERY item @author @liza-pohranichna
		return value
		...
		// if value is object, we just need to go deeper into the object
