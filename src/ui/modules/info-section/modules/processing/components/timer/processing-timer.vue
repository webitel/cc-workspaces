...
const showRenewalButton = computed(() => {
	return (
		processingSecLeft.value <= props.renewalSec && remainingProlongations.value
	);
});
...
const progressColor = computed(() => {
	if (processingProgressSec.value / processingEndSec.value < 0.5)
		return 'success';
	if (processingProgressSec.value / processingEndSec.value < 0.75)
		return 'primary';
	return 'danger';
});
