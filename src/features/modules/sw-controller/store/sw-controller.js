...
const actions = {
	INITIALIZE: async (context) => {
		subscriber = (payload) =>
			context.dispatch('WORKER_MESSAGE_HANDLER', payload);
	...
	SEND_NOTIFICATION: async (context, { body, title, actions }) => {
	...
	HIDE_NOTIFICATIONS: async (context, { title }) => {
...
const swController = new BaseStoreModule().getModule({
	actions,
});
