...
export const useAppNotification = () => {
  ...
  const isAnyRinging = computed(() => store.getters['features/call/IS_ANY_RINGING']);
  const isHangupSoundAllowed = computed(() => store.state.features.notifications.isHangupSoundAllowed);
  ...
  const playRinging = () => store.dispatch('features/callNotifications/HANDLE_ANY_CALL_RINGING');
