...
const formatMessageFile = (file) => {
	// added field 'mime' and 'url' to message`s file object if message dosen't have
	if (!file) return null;
	return {
		...file,
...
export const formatChatMessages = (messages) => {
	// make chat-history messages more similar to current-chat messages
	return messages
		.map((item) => {
			return {
				...item,
...
export const isSelfSideChatMessage = (message) => {
	const isSelfMessage =
		message.member?.self || message.member?.type === 'webitel';
