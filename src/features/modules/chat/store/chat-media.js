...
const getters = {
	CHAT_ON_WORKSPACE: (state, getters, rootState, rootGetters) =>
...
const actions = {
	...
	INITIALIZE_CHAT_PLAYERS: (
		context,
		{ player, chat = context.getters.CHAT_ON_WORKSPACE },
	) => {
		chat.players = player
			? [
					player,
	...
	ATTACH_PLAYER_TO_CHAT: (
		...
		if (chat.players) {
			...
			context.dispatch('INITIALIZE_CHAT_PLAYERS', {
				player,
				chat,
			});
		}
		player.on('play', () => {
			context.dispatch('PAUSE_ALL_CHAT_PLAYERS_EXCEPT', {
				player,
			});
		});
	},
	...
	CLEAN_CHAT_PLAYERS: (
		context,
		{ chat = context.getters.CHAT_ON_WORKSPACE } = {},
	) => {
		/*
