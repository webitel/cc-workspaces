...
describe('features/chat store client handlers: actions', () => {
	...
	it('HANDLE_INVITE_ACTION is called after Invite event', async () => {
		...
		expect(context.dispatch).toHaveBeenCalledWith('HANDLE_INVITE_ACTION', {
			action: ChatActions.UserInvite,
			chat,
		});
	});
	...
	it('HANDLE_MESSAGE_ACTION is called after Message event', async () => {
		...
		expect(context.dispatch).toHaveBeenCalledWith('HANDLE_MESSAGE_ACTION', {
	...
	it('HANDLE_INVITE_ACTION dispatches HANDLE_CHAT_EVENT action with action and chat', async () => {
		await chatModule.actions.HANDLE_INVITE_ACTION(context, {
			action: ChatActions.UserInvite,
			chat,
		});
		expect(context.dispatch).toHaveBeenCalledWith('HANDLE_CHAT_EVENT', {
			action: ChatActions.UserInvite,
			chat,
		});
	});
	...
	it('HANDLE_MESSAGE_ACTION dispatches CHAT_INSERT_TO_START with passed message', async () => {
		await chatModule.actions.HANDLE_MESSAGE_ACTION(context, {
			action: ChatActions.Message,
			chat,
		});
	...
	it('HANDLE_DESTROY_ACTION is called after Destroy event', async () => {
		...
		expect(context.dispatch).toHaveBeenCalledWith('HANDLE_DESTROY_ACTION', {
			action: ChatActions.Destroy,
			chat,
		});
	});
	...
	it('HANDLE_CLOSE_ACTION dispatches HANDLE_CHAT_EVENT action with action and chat params', async () => {
		await chatModule.actions.HANDLE_CLOSE_ACTION(context, {
