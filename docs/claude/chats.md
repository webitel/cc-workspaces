...
- **`closed`** with submodules `unprocessed` and `processed` — REST `AgentChatsAPI`,
  ...
  resolve a closed chat while post-processing is running (WTEL-9955). The request is cut at
  `chat.closedAt` through `offsetDate`: a transferred dialog lives on for the receiving
  agent, and the closing agent sees his own leg only (WTEL-10384).
  With a contact, messages come from the archive instead — see `chatHistory` below.
